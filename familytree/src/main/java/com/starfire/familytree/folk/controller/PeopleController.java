...
public class PeopleController {
	...
	public OrgChartVO getFamilyTree(@RequestBody Map<String, String> param) {
		...
		People husband = peopleService.getFamilyTree(param);// 获取丈夫
		Integer generations = husband.getGenerations();
		Long fatherId = husband.getId();// 丈夫的父亲Id
		Long husbandId = husband.getId(); // 丈夫id
		//获取妻子
		People wife = partnerService.getWife(husbandId);
		if (wife != null) {
			OrgChartItemVO orgChartItemVO = convertOrgChartItemVO(husbandId, wife);// 对数据进行封装转换
			String avatar = wife.getAvatar();
			String brief = wife.getBrief();
			GenderEnum gender = wife.getGender();// 得到性别
		...
		loopChildren(orgChartVO, husband, wife);// 获取妻子和丈夫的孩子，数据存储在orgChartVO对象中
		String fullName = husband.getFullName();
		String brief = husband.getBrief();
	...
	private void loopChildren(OrgChartVO orgChartVO, People father, People mother) {
		...
		for (int j = 0; j < childrenList.size(); j++) {//只要存在孩子后代，就不断的轮询查询数据
